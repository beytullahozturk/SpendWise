...
import { Target, Wallet, AlertTriangle, CheckCircle, Edit2, TrendingUp, PiggyBank } from 'lucide-react';
...
export default function BudgetPage({ user }) {
    ...
    // Filter date state (current month by default)
    ...
    const handleSetBudget = async (category) => {
        ...
        const limit = prompt(`${category} için yeni aylık bütçe limiti girin:`, currentLimit);

        if (limit !== null && limit.trim() !== '') {
            const numLimit = parseFloat(limit);
            if (!isNaN(numLimit) && numLimit >= 0) {
                await setDoc(doc(db, 'budgets', `${user.uid}_${category}`), {
                    uid: user.uid,
                    category,
                    limit: numLimit
                });
